@testset "Euclidean graphs" begin
    N = 10
    d = 2
    # Euclidean is not type-stable.
    g, weights, points = SG.SimpleGraph(SGGEN.Euclidean(N, d, return_weights=true, return_points=true))
    @test nv(g) == N
    @test ne(g) == N * (N - 1) ÷ 2
    ...
    g, weights, points = SG.SimpleGraph(SGGEN.Euclidean(N, d, periodic=true, return_weights=true, return_points=true))
    @test maximum(x -> x[2], weights) <= sqrt(d / 2)
    @test minimum(x -> x[2], weights) >= 0.
    @test maximum(points) <= 1
    @test minimum(points) >= 0.

    @test_throws DomainError SGGEN.Euclidean(points, L=0.01, periodic=true)
    ...
    @testset "Euclidean graph with vertex of highest index isolated" begin
        ...
        g = SG.SimpleGraph(SGGEN.Euclidean(matrix, cutoff=5.0, return_weights=false, return_points=false))
