...
@testset "BFS" begin
    ...
    @testset "geodesic distances using House Graph" begin
    ...
    @testset "is_bipartite with Simple Graph with 5 vertices" begin
        gx = SimpleGraph(5)
    ...
    @testset "bfs_tree and bfs_parents with House graph" begin
        for g in testgraphs(g6)
            n = nv(g)
    ...
    @testset "has_path using House Graph" begin
        gx = SimpleGraph(6)
        d = nv(gx)
