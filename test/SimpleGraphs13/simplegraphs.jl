...
@testset "SG.SimpleGraphs" begin
    ...
    @test @inferred(ne(SG.SimpleGraph(SG.path_digraph(5)))) == 4
    ...
    @test @inferred(ne(SG.SimpleDiGraph(SG.path_graph(5)))) == 8
    ...
    @testset "$g" for g in testgraphs(gx)
        ...
        gc = copy(g)
        @test @inferred(SG.add_edge!(gc, 4 => 1)) && gc == SG.cycle_graph(4)
        @test @inferred(has_edge(gc, 4 => 1)) && has_edge(gc, 0x04 => 0x01)
        gc = copy(g)
        @test @inferred(SG.add_edge!(gc, (4, 1))) && gc == SG.cycle_graph(4)
        @test @inferred(has_edge(gc, (4, 1))) && has_edge(gc, (0x04, 0x01))
        gc = copy(g)
        @test SG.add_edge!(gc, 4, 1) && gc == SG.cycle_graph(4)
    ...
    @testset "$g" for g in testdigraphs(gdx)
        ...
        gc = copy(g)
        @test @inferred(SG.add_edge!(gc, 4 => 1)) && gc == SG.cycle_digraph(4)
        @test @inferred(has_edge(gc, 4 => 1)) && has_edge(gc, 0x04 => 0x01)
        gc = copy(g)
        @test @inferred(SG.add_edge!(gc, (4, 1))) && gc == SG.cycle_digraph(4)
        @test @inferred(has_edge(gc, (4, 1))) && has_edge(gc, (0x04, 0x01))
    ...
    @testset "Constructors from edge lists" begin
        g_undir = SG.erdos_renyi(200, 100; seed=0)
        ...
        g_dir = SG.erdos_renyi(200, 100; is_directed=true, seed=0)
    ...
    let
        g_undir = SG.complete_graph(5)
        g_dir = SG.complete_digraph(5)
        @testset "$g" for g in (testgraphs(g_undir) ∪ testdigraphs(g_dir))
            ...
            @test g4 == (is_directed(g) ? SG.complete_digraph(T(4)) : SG.complete_graph(T(4)))
            @test vmap == [1, 2, 4, 5]
            ...
            @test g2 == (is_directed(g) ? SG.complete_digraph(T(2)) : SG.complete_graph(T(2)))
            @test sort(vmap) == [3, 5]
        ...
        g_undir = SG.erdos_renyi(10, 0.5)
        g_dir = SG.erdos_renyi(10, 0.5, is_directed=true)
        for u = 1:2:10
