@testset "Euclidean graphs" begin
    N = 10
    d = 2
    g, weights, points = @inferred(SG.euclidean_graph(N, d))
    @test nv(g) == N
    @test ne(g) == N * (N - 1) ÷ 2
    ...
    g, weights, points = @inferred(SG.euclidean_graph(N, d, bc=:periodic))
    ...
    @test_throws DomainError SG.euclidean_graph(points, L=0.01,  bc=:periodic)
    @test_throws ArgumentError SG.euclidean_graph(points, bc=:badbc)
    ...
    @testset "Euclidean graph with vertex of highest index isolated" begin
        ...
        g, _ = SG.euclidean_graph(matrix, cutoff= 5.0)
