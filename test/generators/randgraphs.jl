@testset "Randgraphs" begin
    ...
    @test @inferred(nv(r1)) == 10
    @test @inferred(ne(r1)) == 20
    @test @inferred(nv(r2)) == 5
    @test @inferred(ne(r2)) == 10

    @test @inferred(Graph(10,20,seed=3)) == Graph(10,20,seed=3)
    @test @inferred(DiGraph(10,20,seed=3)) == DiGraph(10,20,seed=3)
    @test @inferred(Graph(10,20,seed=3)) == erdos_renyi(10,20,seed=3)
    @test @inferred(ne(Graph(10,40,seed=3))) == 40
    @test @inferred(ne(DiGraph(10,80,seed=3))) == 80

    er = erdos_renyi(10, 0.5)
    @test @inferred(nv(er)) == 10
    @test @inferred(is_directed(er)) == false
    er = erdos_renyi(10, 0.5, is_directed=true)
    @test @inferred(nv(er)) == 10
    @test @inferred(is_directed(er)) == true
    ...
    @test @inferred(nv(ws)) == 10
    @test @inferred(ne(ws)) == 20
    @test @inferred(is_directed(ws)) == false

    ws = watts_strogatz(10, 4, 0.2, is_directed=true)
    @test @inferred(nv(ws)) == 10
    @test @inferred(ne(ws)) == 20
    @test @inferred(is_directed(ws)) == true

    ba = barabasi_albert(10, 2)
    @test @inferred(nv(ba)) == 10
    @test @inferred(ne(ba)) == 16
    @test @inferred(is_directed(ba)) == false
    ...
    @test @inferred(nv(ba)) == 10
    @test @inferred(ne(ba)) == 12
    @test @inferred(is_directed(ba)) == false

    ba = barabasi_albert(10, 2, complete=true)
    @test @inferred(nv(ba)) == 10
    @test @inferred(ne(ba)) == 17
    @test @inferred(is_directed(ba)) == false
    ...
    @test @inferred(nv(ba)) == 10
    @test @inferred(ne(ba)) == 18
    @test @inferred(is_directed(ba)) == false

    ba = barabasi_albert(10, 2, is_directed=true)
    @test @inferred(nv(ba)) == 10
    @test @inferred(ne(ba)) == 16
    @test @inferred(is_directed(ba)) == true
    ...
    @test @inferred(nv(ba)) == 10
    @test @inferred(ne(ba)) == 24
    @test @inferred(is_directed(ba)) == true

    fm = static_fitness_model(20, rand(10))
    @test @inferred(nv(fm)) == 10
    @test @inferred(ne(fm)) == 20
    @test @inferred(is_directed(fm)) == false

    fm = static_fitness_model(20, rand(10), rand(10))
    @test @inferred(nv(fm)) == 10
    @test @inferred(ne(fm)) == 20
    @test @inferred(is_directed(fm)) == true

    sf = static_scale_free(10, 20, 2.0)
    @test @inferred(nv(sf)) == 10
    @test @inferred(ne(sf)) == 20
    @test @inferred(is_directed(sf)) == false

    sf = static_scale_free(10, 20, 2.0, 2.0)
    @test @inferred(nv(sf)) == 10
    @test @inferred(ne(sf)) == 20
    @test @inferred(is_directed(sf)) == true

    rr = random_regular_graph(5, 0)
    @test @inferred(nv(rr)) == 5
    @test @inferred(ne(rr)) == 0
    @test @inferred(is_directed(rr)) == false

    rd = random_regular_digraph(10,0)
    @test @inferred(nv(rd)) == 10
    @test @inferred(ne(rd)) == 0
    @test is_directed(rd)

    rr = random_regular_graph(6, 3, seed=1)
    @test @inferred(nv(rr)) == 6
    @test @inferred(ne(rr)) == 9
    @test @inferred(is_directed(rr)) == false

    rr = random_regular_graph(1000, 50)
    @test @inferred(nv(rr)) == 1000
    @test @inferred(ne(rr)) == 25000
    @test @inferred(is_directed(rr)) == false
    ...
    @test @inferred(nv(rr)) == 10
    @test @inferred(ne(rr)) == 15
    @test @inferred(is_directed(rr)) == false
    num2 = 0; num4 = 0
    for v in vertices(rr)
        d = degree(rr, v)
        @test @inferred( d) == 2 || d == 4
        d == 2 ? num2 += 1 : num4 += 1
    end
    @test @inferred(num4) == 5
    @test @inferred(num2) == 5
    ...
    @test @inferred(nv(rr)) == 3
    @test @inferred(ne(rr)) == 3
    @test @inferred(is_directed(rr)) == false

    rd = random_regular_digraph(1000, 4)
    @test @inferred(nv(rd)) == 1000
    @test @inferred(ne(rd)) == 4000
    @test is_directed(rd)
    @test @inferred(std(outdegree(rd))) == 0

    rd = random_regular_digraph(1000, 4, dir=:in)
    @test @inferred(nv(rd)) == 1000
    @test @inferred(ne(rd)) == 4000
    @test is_directed(rd)
    @test @inferred(std(indegree(rd))) == 0

    rr = random_regular_graph(10, 8, seed=4)
    @test @inferred(nv(rr)) == 10
    @test @inferred(ne(rr)) == 40
    @test @inferred(is_directed(rr)) == false
    for v in vertices(rr)
        @test degree(rr, v) == 8
    end
