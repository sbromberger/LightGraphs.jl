@testset "Betweenness" begin
    ...
    @testset "weighted graph" begin
        ...
        @test isapprox(
            betweenness_centrality(g, vertices(g), distmx; normalize = false),
        ...
        @test isapprox(
            betweenness_centrality(g, vertices(g), distmx; normalize = true),
        ...
        @testset "$g" for g in testdigraphs(a2)
            ...
            @test isapprox(
                betweenness_centrality(g, vertices(g), distmx2; normalize = false),
                [0.0, 1.0, 0.0],
            )
            @test isapprox(
            ...
            @test isapprox(
                betweenness_centrality(g, vertices(g), distmx2; normalize = true),
                [0.0, 0.5, 0.0],
            )
            @test isapprox(
