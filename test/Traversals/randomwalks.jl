@testset "Random walks" begin
    ...
    for g in testdigraphs(gx)
        @test @inferred(LT.walk(g, 1, LT.RandomWalk(nonbacktracking = false, niter = 5))) ==
              [1:5;]
        @test @inferred(LT.walk(
            g,
            2,
            LT.RandomWalk(nonbacktracking = false, niter = 100),
        )) == [2:10;]
        @test_throws BoundsError LT.walk(
            g,
            20,
            LT.RandomWalk(nonbacktracking = false, niter = 20),
        )
        @test @inferred(LT.walk(
            g,
            10,
            LT.RandomWalk(nonbacktracking = true, niter = 20),
        )) == [10]
        @test @inferred(LT.walk(g, 1, LT.RandomWalk(nonbacktracking = true, niter = 20))) ==
              [1:10;]
    end
    ...
    for g in testgraphs(gx)
        visited =
            @inferred(LT.walk(g, 1, LT.RandomWalk(nonbacktracking = true, niter = 20)))
