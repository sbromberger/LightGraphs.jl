@testset "Diffusion Simulation" begin
    ...
    for g in testgraphs(gx)  # this makes graphs of different eltypes
        ...
        @test @inferred(LT.diffusion_rate(g, 1.0, 4, watch=collect(1:5), initial_infections=[2])) == [1, 5, 5, 5]

        # Watching unconnected vertices
        @test @inferred(LT.diffusion_rate(g, 1.0, 4, watch=collect(6:10), initial_infections=[2])) == [0, 0, 0, 0]

        # Watch subset
        @test @inferred(LT.diffusion_rate(g, 1.0, 4, watch=collect(1:2), initial_infections=[2])) == [1, 2, 2, 2]

        @test @inferred(LT.diffusion_rate(g, 1.0, 4, watch=collect(1:5), initial_infections=[10])) == [0, 0, 0, 0]
