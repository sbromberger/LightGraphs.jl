...
const SGC = LightGraphs.SimpleGraphsCore
const SG = LightGraphs.SimpleGraphs
const SGGEN = LightGraphs.SimpleGraphs.Generators

import LightGraphs.SimpleGraphsCore: fadj, badj, adj
import LightGraphs.edgetype, LightGraphs.has_edge
import LightGraphs.Connectivity: is_graphical
...
const simplegraphcoretestdir = dirname(@__FILE__)

const simplecore_tests = [
    "simplegraphs",
    "simpleedge",
    "simpleedgeiter"
]
...
@testset "LightGraphs.SimpleGraphs" begin
    for t in simplecore_tests
        tp = joinpath(simplegraphcoretestdir, "$(t).jl")
        include(tp)
    end
end
