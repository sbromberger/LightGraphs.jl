@testset "Parallel.Bellman Ford" begin
    ...
    for g in testdigraphs(g4)
        y = @inferred(Parallel.bellman_ford_shortest_paths(g, [2], d1))
        z = @inferred(Parallel.bellman_ford_shortest_paths(g, [2], d2))
        ...
        @test @inferred(enumerate_paths(z))[2] == []
        @test @inferred(enumerate_paths(z))[4] == enumerate_paths(z, 4) == [2, 3, 4]
        z = @inferred(Parallel.bellman_ford_shortest_paths(g, [2]))
    ...
    for g in testgraphs(gx)
        d = [1 -3 1; -3 1 1; 1 1 1]
        @test_throws LightGraphs.ShortestPaths.NegativeCycleError Parallel.bellman_ford_shortest_paths(g, [1], d)
