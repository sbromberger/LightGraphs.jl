@testset "Parallel.Betweenness" begin
    ...
    for g in testdigraphs(gint)
        ...
        yt = Parallel.betweenness_centrality(
            g,
            endpoints = true,
            normalize = false,
            parallel = :threads,
        ...
        yd = Parallel.betweenness_centrality(
            g,
            endpoints = true,
            normalize = false,
        ...
        xt2 = @inferred(Parallel.betweenness_centrality(
            g,
            collect(1:20);
            parallel = :threads,
        ))
        @test length(xt2) == 50
        xd2 = @inferred(Parallel.betweenness_centrality(
            g,
            collect(1:20);
            parallel = :distributed,
        ))
    ...
    @test @inferred(Parallel.betweenness_centrality(s1; parallel = :distributed)) ==
          [0, 1, 0]

    @test @inferred(Parallel.betweenness_centrality(s2; parallel = :threads)) == [0, 0.5, 0]
    @test @inferred(Parallel.betweenness_centrality(s2; parallel = :distributed)) ==
          [0, 0.5, 0]
