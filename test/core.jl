@testset "Core" begin
    ...
    for g in testgraphs(g5w)
        @test @inferred(in_degree(g, 1)) == @inferred(out_degree(g, 1)) == @inferred(degree(g, 1)) == 4
        @test @inferred(in_degree(g)) == @inferred(out_degree(g)) == @inferred(degree(g)) == [4, 3, 3, 3, 3]
        ...
        z2 = @inferred(degree_histogram(g, in_degree))
        z3 = @inferred(degree_histogram(g, out_degree))
    ...
    for g in testdigraphs(g5wd)
        @test @inferred(in_degree(g, 2)) == 2
        @test @inferred(out_degree(g, 2)) == 1
        @test @inferred(degree(g, 2)) == 3
        @test @inferred(in_degree(g)) == [0, 2, 2, 2, 2]
        @test @inferred(out_degree(g)) == [4, 1, 1, 1, 1]
