@testset "Closeness" begin
    ...
    for g in testdigraphs(g5)
        y = @inferred(LCENT.centrality(g, LCENT.Closeness(normalize=false)))
        z = @inferred(LCENT.centrality(g, LCENT.Closeness()))
    ...
    for g in testdigraphs(a2)
        ...
        y = @inferred(LCENT.centrality(g, distmx2, LCENT.Closeness(normalize=false)))
        z = @inferred(LCENT.centrality(g, distmx2, LCENT.Closeness()))
        @test isapprox(y, c2)
        @test isapprox(z, c2)
    end
    ...
    for g in testgraphs(a1)
        ...
        y = @inferred(LCENT.centrality(g, distmx1, LCENT.Closeness(normalize=false)))
        z = @inferred(LCENT.centrality(g, distmx1, LCENT.Closeness()))
        @test isapprox(y, c1)
        @test isapprox(z, c1)
    end
end
