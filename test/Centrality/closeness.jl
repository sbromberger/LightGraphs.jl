@testset "Closeness" begin
    ...
    for g in testdigraphs(g5)
        y = @inferred(centrality(g, Closeness(normalize=false)))
        z = @inferred(centrality(g, Closeness()))
    ...
    for g in testdigraphs(a2)
        ...
        y = @inferred(centrality(g, distmx2, Closeness(normalize=false)))
        z = @inferred(centrality(g, distmx2, Closeness()))
        @test isapprox(y, c2)
        @test isapprox(z, c2)
    end
    ...
    for g in testgraphs(a1)
        ...
        y = @inferred(centrality(g, distmx1, Closeness(normalize=false)))
        z = @inferred(centrality(g, distmx1, Closeness()))
        @test isapprox(y, c1)
        @test isapprox(z, c1)
    end
end
