@testset "LinearSystemPageRank" begin
    ...
    for α in [0.75, 0.85]
        ...
        @testset "$g" for g in testdigraphs(g5)
            @test LCENT.centrality(g, LCENT.LinearSystemPageRank())[3] ≈ 0.318 atol = 0.001
            @test length(@inferred(LCENT.centrality(g, LCENT.LinearSystemPageRank()))) == nv(g)
            @test_throws Centrality.ConvergenceError LCENT.centrality(g, LCENT.LinearSystemPageRank(α=2))
            @test_throws Centrality.ConvergenceError LCENT.centrality(g, LCENT.LinearSystemPageRank(α=α, n=1))
            @test isapprox(LCENT.centrality(g, LCENT.LinearSystemPageRank(α=α)), dense_pagerank_solver(g, α), atol=0.001)
        end
        ...
        @testset "$g" for g in testgraphs(g6)
            ...
            @test_throws Centrality.ConvergenceError LCENT.centrality(g, LCENT.PageRank(α=α, n=1))
