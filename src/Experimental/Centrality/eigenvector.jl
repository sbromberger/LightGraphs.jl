...
struct Eigenvector <: CentralityAlgorithm

centrality(g::AbstractGraph; alg::Eigenvector) = abs.(vec(eigs(adjacency_matrix(g), which=LM(), nev=1)[2]))::Vector{Float64}
