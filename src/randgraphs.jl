...
doc"""
`stochastic_block_model(c::Matrix{Float64}, n::Vector{Int})`
`stochastic_block_model(cin::Float64, coff::Float64, n::Vector{Int})`
...
related functions.
"""
function stochastic_block_model{T}(c::Matrix{T}, n::Vector{Int}; seed::Int = -1)
    ...
    for a=1:K
        ra = cum[a]+1:cum[a+1]
        for b=a:K
            @assert a==b? c[a,b] <= n[b]-1 : c[a,b] <= n[b]
...
function stochastic_block_model{T}(cint::T, cext::T, n::Vector{Int})
    K = length(n)
    c = [ifelse(a==b, cint, cext) for a=1:K,b=1:K]
    stochastic_block_model(c, n)
end

doc"""StochasticBlockModel(n,nodemap,affinities)
...
doc"""A constructor for StochasticBlockModel that uses the sizes of the blocks
...
doc"""Produce the sbm affinity matrix where the external probabilities are the same
...
doc"""Construct the affinity matrix for a near bipartite SBM.
...
doc"""Return a generator for edges from a stochastic block model near-bipartite graph."""
...
doc"""Generates a stream of random pairs in 1:n"""
...
doc"""Counts the number of edges that go between each block"""
