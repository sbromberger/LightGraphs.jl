...
function non_backtracking_randomwalk(g::AbstractGraph, s::Integer, niter::Integer; seed::Int=-1)
    Base.depwarn("`non_backtracking_randomwalk` is deprecated. Equivalent functionality has been moved to `LightGraphs.Traversals.walk`.", :non_backtracking_randomwalk)
    rng = LightGraphs.getRNG(seed)
...
function self_avoiding_walk(g::AG, s::Integer, niter::Integer; seed::Int=-1) where AG <: AbstractGraph{T} where T
    Base.depwarn("`self_avoiding_walk` is deprecated. Equivalent functionality has been moved to `LightGraphs.Traversals.walk`.", :self_avoiding_walk)
    rng = getRNG(seed)
