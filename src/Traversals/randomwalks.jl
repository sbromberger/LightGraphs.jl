...
"""
function randomwalk(g::AG, s::Integer, niter::Integer; rng=GLOBAL_RNG) where AG <: AbstractGraph{T} where T
...
@traitfn function non_backtracking_randomwalk(g::AG::(!IsDirected), s::Integer, niter::Integer; rng=GLOBAL_RNG) where {T, AG<:AbstractGraph{T}}
    s in vertices(g) || throw(BoundsError())
    visited = Vector{T}()
    sizehint!(visited, niter)
    currs = s
...
"""
function self_avoiding_walk(g::AG, s::Integer, niter::Integer; rng=GLOBAL_RNG) where AG <: AbstractGraph{T} where T
    s in vertices(g) || throw(BoundsError())
    visited = Vector{T}()
    svisited = Set{T}()
    sizehint!(visited, niter)
