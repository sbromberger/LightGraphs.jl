...
SimpleGraph(ne, fadjlist::Vector{Vector{T}}) where {T} = SimpleGraph{T}(ne, fadjlist)
...
function SimpleGraphFromIterator(iter)::SimpleGraph
    ...
    if Base.IteratorEltype(iter) == Base.HasEltype()
        ...
        if (E <: SimpleGraphEdge{<: Integer} && isconcretetype(E))
            T = eltype(E)
            isconcretetype(T) && return _SimpleGraphFromIterator(iter, T)
        end
    end
...
function squash(g::SimpleGraph)
    validtypes = [UInt8, UInt16, UInt32, UInt64]
    nvg = nv(g)
    for T in validtypes
        nvg < typemax(T) && return SimpleGraph{T}(g)
    end
end
