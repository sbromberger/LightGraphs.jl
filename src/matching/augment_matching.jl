...
"""
    augment_matching(g, matching=Vector())

Algorithm to improve (Increase the size of) a `matching`.
...
### Implementation Notes
It assumes `matching` is a valid matching. 
i.e. Each edge in `matching` is present in `g` and no two edges have a common end point.
### Performance
...
function augment_matching(g::AbstractGraph{T},
	...
	sizehint!(unpartnered, nvg-size(matching)[1])
	for v in vertices(g)
		if partner[v] == zero(T)
			push!(unpartnered, v)
		end
