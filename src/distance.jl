...
eccentricity{T}(g::AbstractGeneralGraph, distmx::AbstractArray{T, 2}) =
    eccentricity(g, vertices(g), distmx)

diameter{T}(all_e::Vector{T}) = maximum(all_e)
diameter{T}(g::AbstractGeneralGraph, distmx::AbstractArray{T, 2} = DefaultDistance()) =
...
periphery{T}(g::AbstractGeneralGraph, distmx::AbstractArray{T, 2} = DefaultDistance()) =
    periphery(eccentricity(g, distmx))

radius{T}(all_e::Vector{T}) = minimum(all_e)
radius{T}(g::AbstractGeneralGraph, distmx::AbstractArray{T, 2} = DefaultDistance()) =
...
center{T}(g::AbstractGeneralGraph, distmx::AbstractArray{T, 2} = DefaultDistance()) =
