...
Return the weight of the edge `(u, v)` in graph `g`, or `0` if the edge does not
exist. For SimpleGraphs, the weight defaults to `1` if the edge exists.

If `u` or `v` ∉ `vertices(g)`, `0` will be returned; no error will be thrown.
"""
getindex(g::AbstractGraph, u::Int, v::Int) = (u > 0) && (v > 0) ? weights(g)[u, v] * has_edge(g, u, v) : zero(eltype(weights(g)))
