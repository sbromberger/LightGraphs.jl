...
A `SimpleCycleAlgorithm` that specifies the use of a limited
length iterativwe algorithm. The algorithm will return after
computing at most `ceiling` cycles of length at most `n` of
the given graph. Both directed and undirected graphs are
supported.
...
LimitedLength(n::Integer) = LimitedLength(n, 1e6)
